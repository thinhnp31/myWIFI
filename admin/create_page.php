    ...
    if ($background_status && $logo_status) {
      ...
      if ($result->num_rows > 0) {
        ...
        $type = "danger";
            ...
            //create page
        $stmt = $conn->prepare("INSERT INTO pages (page_name, background, logo, connection_type, profile_id, welcome, ad_url, youtube_link, countdown, survey_id) VALUES (?,?,?,?,?,?,?,?,?,?) ");          
        $stmt->bind_param('ssssisssii', $page_name, $background_name, $logo_name, $connection_type, $profile_id, $welcome, $ad_url, $youtube_link, $countdown, $survey_id);
        $stmt->execute();
